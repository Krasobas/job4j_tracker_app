...
public class HQLUsage {
    ...
    public static void unique(Session session) {
        Query<Item> query = session.createQuery(
                "from Item as i where i.id = 6", Item.class);
        System.out.println(query.uniqueResult());
    }

    public static void listAll(Session session) {
        Query<Item> query = session.createQuery("from ru.job4j.tracker.model.Item", Item.class);
